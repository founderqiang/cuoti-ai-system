...
6. 【纯文本无图排版，禁用LaTeX】：坚决不使用 SVG、Python 绘图代码、Markdown 代码块或任何程序化作图内容。若题目涉及几何图形，只能使用清晰、完整、可操作的中文文字描述图形关系，并补上一句固定提醒：“**注：由于技术限制，图形请根据以上描述自行规范绘制。**”【极其重要】：绝不能使用任何 LaTeX 数学代码（严禁出现 `\frac`, `\Rightarrow` 等）！所有的数学公式必须写成键盘能直接敲出的纯字符形式（例如使用 `b/(2a)`，`x^2`，`=>`），保证导出到普通 Word 后肉眼直接可读。
                ...
                for chunk in completion:
                    if hasattr(chunk, 'choices') and len(chunk.choices) > 0:
                        ...
                        if getattr(chunk.choices[0], 'finish_reason', None) == 'length':
                            suggested = max(1, count - 2)
                            yield f"\n\n⚠️ **[系统提示：大模型生成已达到单次最大长度限制被强制截断，建议将生成题数改为{suggested}道重试]**"
            except Exception as e:
                import traceback
                traceback.print_exc()
                yield f"\n\n[后台生成出错: {str(e)}]"
