...
7. 【详略得当与防止截断】：为了防止输出过长被断开，必须严格控制节奏：每道题的【详细解析】需保持在 150-250 字左右。既要清晰地展示核心等式、关键定理和主干推导步骤以保证学生能看懂，也要点到为止，【绝对禁止】过度发散、举无关特例或啰嗦计算细节！必须且只能生成 {count} 道题目，满 {count} 题立刻收尾。
